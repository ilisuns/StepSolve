...
export class AppController {
  ...
  async solve(@Body() body: SolveBody) {
    ...
    if (!question) {
      return {
        action: 'solve',
        answer: 'Judgment: No problem found yet.\n\nCurrent step:Please drop your homework problem into StepSolve first.\n\nNext step:Add the original problem, your step, or where you got stuck.',
      };
    }
    ...
    if (linearMatch) {
      ...
      if (Number.isFinite(a) && Number.isFinite(b) && Number.isFinite(c) && a !== 0) {
        ...
        const actionText = b > 0 ? 'subtract ' + absB : 'add ' + absB;
        ...
        return {
          ...
          answer: [
            'Judgment: Keep going.',
            '',
            'Current step: First deal with the number next to x: ' + sideText + '. To leave x by itself, move that number away first.',
            '',
            'Next step: Do the same operation on both sides: ' + actionText + ' to get ' + a + 'x = ' + right + '.'
          ].join('\n'),
        };
      }
    }
    ...
    const prompt = [
      'IMPORTANT: Always answer in English only.Use English labels only.固定使用中文标签：Judgment:、Current step:、Next step:',
      'Always answer in English only.Use English labels only.固定使用：Judgment:、Current step:、Next step:、Hint:.你是一个面向学生的作业拆解助手.',
      '产品不是答案机，目标是把作业变成学生能继续走下去的学习路径.',
      '只服务数学、物理、化学.`r`nAnswer in English only，Use short, clear English for middle-school students.必须严格使用这个格式：Judgment:……\n\nCurrent step:……\n\nNext step:…….Never use Chinese，尤其禁止出现 Judgment、当前这一步、下一步、、、、.',
      '',
  ...
  async followUp(@Body() body: FollowBody) {
    ...
    if (!question) {
      return {
        action,
        answer: 'Judgment:还没有看到原题.\n\nCurrent step:请先把原题放进解池.\n\nNext step:原题、步骤、卡住点都可以一起放进来.',
      };
    }
    ...
    const actionInstruction =
      action === 'hint'
        ? 'The student clicked Get a hint.只给方向，不要代做，不要给完整答案.'
        : action === 'check'
          ? 'The student clicked Check this step.必须先检查学生步骤是否成立.明显算错、移项错、公式错、单位错，必须直接判“这里要改”，不能说“可以继续”.'
          : 'The student clicked Next step.只承接当前内容，给下一小步，不能重新从头拆题.';

    const prompt = [
      'IMPORTANT: Always answer in English only.Use English labels only.固定使用中文标签：Judgment:、Current step:、Next step:',
      'Always answer in English only.Use English labels only.固定使用：Judgment:、Current step:、Next step:、Hint:.你是一个面向学生的作业拆解助手.',
      '产品不是答案机，目标是让学生继续走下一步.Answer in English only，Use short, clear English for middle-school students.必须严格使用这个格式：Judgment:……\n\nCurrent step:……\n\nNext step:…….Never use Chinese，尤其禁止出现 Judgment、当前这一步、下一步、、、、.`r`nAnswer in English only，Use short, clear English for middle-school students.必须严格使用这个格式：Judgment:……\n\nCurrent step:……\n\nNext step:…….Never use Chinese，尤其禁止出现 Judgment、当前这一步、下一步、、、、.',
      '',
      '三科强引导规则：数学、物理、化学里，只要关系链容易绕，即使题目看起来基础，也按困难题或强引导题处理.',
      '',
      'V1自适应规则块：',
      ...
      '5. 如果 action 是 next 或 hint，且没有明确学生步骤，判断栏只能写“继续拆解”或“Here is a hint”，禁止写“这一步基本对”“还差验证”“这一步不对”.',
      '',
      '数学表达补充：',
      '数学一元一次方程表达规则：如果已经得到 2x = 8 或 ax = b，下一步应引导“等式两边同时除以系数”，不要说“将系数与未知数相乘”.',
      '',
    ...
    try {
      ...
      const neutralJudge = action === 'hint' ? 'Judgment: Here is a hint.' : 'Judgment: 继续拆解.';
      const answer =
      ...
      return {
        action,
        answer: physicsSpeedOrderedAnswer.replace(/Current step:\s*\n\s*(Next step:\s*\n?\s*([^\n]+))/, 'Current step:$2\n\n$1'),
      };
    } catch (error) {
