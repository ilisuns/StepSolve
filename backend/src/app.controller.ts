...
export class AppController {
  ...
  async followUp(@Body() body: FollowBody) {
    ...
    const prompt = [
      ...
      '强引导链路：已知条件 → 目标 → 关系/规则/公式 → 代入/转化 → 计算/推导 → 单位/意义/检查。',
      '不能假设学生自然能接上关系链；不要直接跳答案，要把中间关系讲清楚。',
      "Math counting special rule: when a counting problem asks total balls from red balls and blue balls, each next step must move forward: red balls + blue balls = total - + 2 - + 2 = 5 - is 5 balls.",
