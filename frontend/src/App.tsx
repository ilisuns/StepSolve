...
function guessSubjectFromText(text: string) {
  if (/氢气|氧气|Chemistry式|方程式|配平|H2|O2|H₂|O₂|H2O|H₂O|原子|元素/.test(text)) return 'Chemistry';
  if (/物体|质量|合力|拉力|加速度|受力|牛顿|速度|距离|时间|m\/s|F\s*=|N\b|kg|m\/s²/.test(text)) return 'Physics';
  return '';
}
...
function App() {
  ...
  const publicBetaHomeworkLimitText = '这道作业今天的互动次数已用完。你可以先Save结果，或换一道题继续测试。';
  ...
  async function handleSolve() {
    ...
    if (hasInstructionNoise) {
      ...
      setNextHint('');
      setCheckResult('');
      addActionLog('Start solving: instructions blocked');
      return;
    }
    ...
    try {
      ...
      addActionLog('Start solving：Request failed');
    } finally {
      setLoadingAction('');
    }
  }
  ...
  function getSaveQuestionText() {
    ...
    const cleanLines = lines.filter((line) => {
      ...
      if (/^\d+Grade\s*\/\s*(Math|Physics|Chemistry)$/.test(line)) return false;
      if (/^\d+Grade$/.test(line)) return false;
      if (/^(Math|Physics|Chemistry)$/.test(line)) return false;
      if (/^Grade[:：]\s*\d+Grade$/.test(line)) return false;
      if (/^Subject[:：]\s*(Math|Physics|Chemistry)$/.test(line)) return false;
      ...
      if (/Clear|解池里|只放|Subject选|Start solving|重新拆解|Next step|Save|Copy\s*\/\s*Save/.test(line)) return false;
      ...
      <section className="heroPanel">
        ...
        <div className="workspace">
          <section className="poolCard">
            ...
            />
            <div className="poolFixedTip">Only put homework content here. Do not add grade, subject, or extra instructions.</div>
            ...
            <div className="resultCard currentStepCard">
              ...
              {currentStep ? (
                ...
                <div className="emptyState">
                  <strong>No current step yet</strong>
                  <p>Drop in a problem. StepSolve will show the easiest first step.</p>
                </div>
              )}
